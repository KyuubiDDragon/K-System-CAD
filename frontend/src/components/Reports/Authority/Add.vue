...
import { defineComponent, reactive, computed, onMounted, watch, ref, type PropType } from 'vue';
import type { Report, Category, ReportCode, Employee, ReportStatus } from '@/types/Report';
import type { PersonFile } from '@/types/Person';
import { apiClientAuth } from '@/api'; // Use configured Axios instance
import { useAuthStore } from '@/stores/auth'; // Import Pinia Auth Store
import TiptapEditor from '@/components/TiptapEditor.vue'; // Import Tiptap Editor component
...
// --- Emits ---
const emit = defineEmits(['update:modelValue', 'reportAdded', 'close']); // Renamed closeForm to close
...
// Update report reactive object
const report = reactive<ExtendedReport>({
    id: 0,
...
const fetchAdditionals = async () => {
    try {
        console.log('Starting additionals fetch...');
        const response = await apiClientAuth.get('/report/?action=getAdditionals');
        console.log('Additionals response:', response.data);
        ...
        console.log('Additionals loaded successfully:', additionals.value.length, 'items');
    } catch (error) {
...
const addNewReport = async () => {
    try {
        await apiClientAuth.post('/report/?action=addReport', {
            ...report,
...
const fetchCategories = async () => {
    try {
        loadingSteps.value.categories = true;
        ...
        // Set report properties based on selected category (if we have categories and a selected one)
        if (categories.value.length > 0 && props.selectedCategory) {
            const category = categories.value.find(cat => cat.id === props.selectedCategory);
            if (category) {
        ...
        loadingSteps.value.categories = false;
    }
};
...
const fetchCompanies = async () => {
    try {
        ...
        companies.value = data.map((company: { id: string | number, name: string }) => ({
            ...company,
...
const fetchEmployees = async () => {
    try {
        ...
        // Auto-fill creator with current user's linked employee
        if (employees.value.length > 0 && authStore.user?.linked_employee) {
            const linkedEmp = employees.value.find(emp => emp.id === authStore.user.linked_employee);
            if (linkedEmp) {
                report.creator = linkedEmp.id;
                console.log("Auto-filled creator with linked employee:", linkedEmp.display_name);
            }
        }
    } catch (error) {
...
const onAdditionalSelected = (index:any, selectedId:any) => {
...
// Init data with props or fetch if needed
onMounted(() => {
    console.log("Add component mounted with props:", {
        selectedCategory: props.selectedCategory,
        categoriesLength: props.categories?.length || 0,
        employeesLength: props.employees?.length || 0,
        personsLength: props.persons?.length || 0,
        companiesLength: props.companies?.length || 0,
        reportCodesLength: props.reportCodes?.length || 0,
        reportStatusesLength: props.reportStatuses?.length || 0,
    });

    // Initialize local refs with prop values if available
    if (Array.isArray(props.categories) && props.categories.length > 0) {
        categories.value = [...props.categories];
        console.log("Using categories from props:", categories.value.length);
    }

    if (Array.isArray(props.employees) && props.employees.length > 0) {
        employees.value = [...props.employees];
        console.log("Using employees from props:", employees.value.length);

        // Auto-fill creator with current user's linked employee
        if (authStore.user?.linked_employee) {
            const linkedEmp = employees.value.find(emp => emp.id === authStore.user.linked_employee);
            if (linkedEmp) {
                report.creator = linkedEmp.id;
                console.log("Auto-filled creator with linked employee:", linkedEmp.display_name);
    ...
    if (Array.isArray(props.persons) && props.persons.length > 0) {
        persons.value = [...props.persons];
        console.log("Using persons from props:", persons.value.length);
    }

    if (Array.isArray(props.companies) && props.companies.length > 0) {
        companies.value = [...props.companies];
        console.log("Using companies from props:", companies.value.length);
    }

    if (Array.isArray(props.reportCodes) && props.reportCodes.length > 0) {
        reportCodes.value = [...props.reportCodes];
        console.log("Using reportCodes from props:", reportCodes.value.length);
    }

    if (Array.isArray(props.reportStatuses) && props.reportStatuses.length > 0) {
        statuses.value = [...props.reportStatuses];
        console.log("Using reportStatuses from props:", statuses.value.length);
    }

    // Ensure the additionals are initialized
    console.log("Explicitly loading additionals on component mount");
    fetchAdditionals();

    // Set category-related data if selected category is provided
    if (props.selectedCategory && categories.value.length > 0) {
        const selectedCat = categories.value.find(cat => cat.id === props.selectedCategory);
        if (selectedCat) {
            console.log("Selected category found, initializing report template:", selectedCat.name);
            report.category_id = selectedCat.id;
            report.text = selectedCat.template || '';
            report.title = selectedCat.title || '';
        } else {
            console.warn("Selected category ID provided but not found in categories list");
        }
    }
});
...
const handleSubmit = async () => {
    ...
    try {
        ...
        Object.keys(customFieldValues).forEach(fieldName => {
            const field = customFields.value.find(f => f.field_name === fieldName);
            if (field) {
                customFieldsData[fieldName] = customFieldValues[fieldName];
            }
        });

        // Filter out additionals without a valid ID
        const validAdditionals = selectedAdditionals.value.filter(
            additional => additional.id !== null && additional.id !== undefined
        );
        ...
        const reportData = {
            ...
            additionals: validAdditionals
        };

        const response = await apiClientAuth.post('/report/index.php?action=addReport', reportData);

        if (response.data.success) {
            showSuccessSnackbar('Bericht erfolgreich erstellt');
            closeDialog();
            router.push('/reports');
        }
    } catch (error) {
        console.error('Fehler beim Erstellen des Berichts:', error);
        showErrorSnackbar('Fehler beim Erstellen des Berichts');
    } finally {
        isLoading.value = false;
    }
};
    ...
    <v-card v-if="dialog && (selectedCategory !== undefined && selectedCategory !== null)" class="report-card" elevation="4">
        ...
        <v-toolbar flat density="compact" color="primary" class="card-toolbar">
            <v-toolbar-title class="text-subtitle-1">
                ...
                {{ report.title ? report.title : $t('newReport') }}
            </v-toolbar-title>
        </v-toolbar>
        ...
        <v-card-actions class="pa-4">
            ...
            <v-btn 
                ...
                variant="elevated"
                @click="handleSubmit"
                :disabled="!isFormValid(report) || isLoading"
                :loading="isLoading"
                class="action-button"
            >
                <v-icon start>mdi-plus</v-icon>
                {{ isLoading ? $t('saving') : 'Hinzufügen' }}
            </v-btn>
        </v-card-actions>
    </v-card>
</template>
